...
namespace GoodDataService.Api
{
	...
	public class MandatoryUserFilterActions : ApiWrapperBase
	{
		// https://secure.gooddata.com/gdc/md/projectid/userfilters
		// https://secure.gooddata.com/gdc/md/PROJECT_ID/query/userfilters

		private static readonly ILog Logger = LogManager.GetLogger(typeof (MandatoryUserFilterActions));
		...
		public string Update(string filterUri, string newTitle, string newExpression)
		{
			var getUrl = Url.Combine(Config.Url, filterUri);
			//var editUrl = Url.Combine(getUrl, "?mode=edit");
			var response = GetRequest(getUrl);
